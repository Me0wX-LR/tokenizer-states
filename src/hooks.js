...
import TokenStates from "./tokenizer/TokenStates.js";
import StateHud from "./tokenizer/StateHud.js";
...
function getAvatarKey() {
  return TokenStates.getAvatarKey();
}

function getAvatarPath(actor) {
  return TokenStates.getAvatarPath(actor);
}
...
async function updateActor(tokenizerResponse) {
  ...
  const stateId = tokenizerResponse.stateId ?? TokenStates.DEFAULT_ID;

  if (tokenizerResponse.actor && !TokenStates.isDefault(stateId)) {
    await TokenStates.saveState(tokenizerResponse.actor, {
      id: stateId,
      name: tokenizerResponse.stateName,
      slug: tokenizerResponse.stateSlug,
      avatar: tokenizerResponse.modifyAvatar === false ? undefined : tokenizerResponse.avatarFilename,
      token: tokenizerResponse.modifyToken === false ? undefined : tokenizerResponse.tokenFilename,
      forceDynamicRing: tokenizerResponse.forceDynamicRing,
    });
    if (TokenStates.getActiveId(tokenizerResponse.actor) === stateId) {
      TokenStates.refreshActorDisplays(tokenizerResponse.actor);
    }
    return;
  }

  if (tokenizerResponse.actor && tokenizerResponse.stateName) {
    await TokenStates.setDefaultName(tokenizerResponse.actor, tokenizerResponse.stateName);
  }
...
function buildStateOptions(actor, token, extra = {}) {
  const state = TokenStates.getState(actor, extra.stateId ?? TokenStates.getActiveId(actor), token);
  return {
    stateId: state.id,
    stateName: state.name,
    stateSlug: state.slug,
    avatarFilename: extra.avatarFilename ?? TokenStates.getAvatarSrc(actor, state.id),
    tokenFilename: extra.tokenFilename ?? TokenStates.getTokenSrc(actor, token, state.id),
  };
}

function tokenizeActor(actor, extra = {}) {
  ...
  const options = {
    ...
    ...buildStateOptions(actor, extra.token, extra),
  };
...
function tokenizeSceneToken(doc, extra = {}) {
  const options = {
    actor: doc.actor,
    token: doc.token,
    name: doc.token.name,
    ...
    ...buildStateOptions(doc.actor, doc.token, extra),
  };
...
function tokenizeSceneTokenV2(doc, extra = {}) {
  const options = {
    actor: doc,
    token: doc.token,
    name: doc.token.name,
    ...
    disposition: doc.token.disposition,
    nameSuffix: `.${doc.token._id}`,
    ...buildStateOptions(doc, doc.token, extra),
  };
...
function tokenizeDoc(doc, extra = {}) {
  if (doc.token) {
    tokenizeSceneToken(doc, extra);
  } else {  
    tokenizeActor(doc, extra);
  }
}
...
function exposeAPI() {
  const API = {
    ...
    autoToken,
    states: {
      get: (actor, token) => TokenStates.getStates(actor, token),
      getActiveId: (doc) => TokenStates.getActiveId(doc),
      switch: (actor, stateId, options) => TokenStates.setActive(actor, stateId, options),
      create: (actor, name) => TokenStates.createState(actor, name),
      delete: (actor, stateId) => TokenStates.deleteState(actor, stateId),
      rename: (actor, stateId, name) => TokenStates.renameState(actor, stateId, name),
    },
  };
